  ...
  return (
    <div>
      ...
      <div className="bg-[var(--bg-surface)] border border-[var(--border-default)] rounded-[var(--radius-lg)] overflow-hidden shadow-[var(--shadow-sm)]">
        ...
        <Suspense key={`${q}|${estado}|${page}`} fallback={<VehiculosTableSkeleton />}>
          <VehiculosTable q={q} estado={estado} page={page} filtrosQuery={filtrosQuery} />
        </Suspense>
      </div>
    </div>
  );
...
async function VehiculosTable({
  ...
  return (
    <>
      <div className="px-4 pt-3 pb-1 text-[13px] text-[var(--text-secondary)]">{total} registro{total === 1 ? "" : "s"} en total</div>
...
function VehiculosTableSkeleton() {
  return (
    <>
      <div className="px-4 pt-3 pb-1"><Skeleton className="h-[13px] w-[150px]" /></div>
      <div className="overflow-x-auto">
        <table className="w-full border-collapse max-[900px]:min-w-[700px]">
          <thead>
            <tr>
              {["Vehículo", "Propietario", "Precio/día", "Año", ""].map((h, i) => (
          ...
          <tbody>
            {Array.from({ length: 5 }).map((_, i) => (
              <tr key={i}>
                <td className={tdClass}>
                  <div className="flex items-center gap-[10px]">
                    <Skeleton className="rounded-[var(--radius-md)] w-12 h-8 shrink-0" />
                    <Skeleton className="h-[13px] w-[110px]" />
                  </div>
                </td>
                <td className={tdClass}><Skeleton className="h-[13px] w-[120px]" /></td>
                <td className={tdClass}><Skeleton className="h-[13px] w-[60px]" /></td>
                <td className={tdClass}><Skeleton className="h-[13px] w-[40px]" /></td>
                <td className={tdClass}>
                  <div className="flex gap-2 justify-end">
