...
function serializar(v: Awaited<ReturnType<typeof findVehiculoById>>) {
  if (!v) return null;
  return { ...v, precio: Number(v.precio) };
}

export async function getVehiculosDisponibles() {
  const vehiculos = await findVehiculosDisponibles();
  return { data: { vehiculos: vehiculos.map(v => ({ ...v, precio: Number(v.precio) })) }, error: null };
}
...
export async function getVehiculo(id: string) {
  ...
  return { data: serializar(vehiculo), error: null };
}

export async function getVehiculosByPropietario(id_propietario: string) {
  const vehiculos = await findVehiculosByPropietario(id_propietario);
  return { data: { vehiculos: vehiculos.map(v => ({ ...v, precio: Number(v.precio) })) }, error: null };
}

export async function crearVehiculo(id_propietario: string, input: CrearVehiculoInput) {
  const vehiculo = await createVehiculo({ id_propietario, ...input });
  return { data: serializar(vehiculo), error: null };
}
...
export async function actualizarVehiculo(id: string, input: ActualizarVehiculoInput) {
  ...
  if (!existente) return { data: null, error: "Vehículo no encontrado" };
